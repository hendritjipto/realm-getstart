...
// Don't use this as your key 
var key = new Int8Array(64);
for (let i = 0; i < 64; i++) {
    key[i] = 1;
}
const hexString = Buffer.from(key).toString('hex');
console.log("KEY : " + hexString);
// Authenticate an anonymous user.
const anonymousUser = await app.logIn(Realm.
    Credentials.emailPassword("encrypt@mongodb.com", "xxxx"));
